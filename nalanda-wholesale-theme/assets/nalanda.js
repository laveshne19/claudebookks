...
/* ---- Wholesale: MOQ quantity steppers ---- */
(function(){
  function ready(fn){ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded', fn); }
  ready(function(){
    document.querySelectorAll('[data-qty]').forEach(function(box){
      ...
      function clamp(){ var v = parseInt(input.value,10); if(isNaN(v)||v<min) v=min; input.value=v; }
      box.querySelector('[data-qty-dec]').addEventListener('click', function(){ input.value = Math.max(min,(parseInt(input.value,10)||min)-1); });
      box.querySelector('[data-qty-inc]').addEventListener('click', function(){ input.value = (parseInt(input.value,10)||min)+1; });
      input.addEventListener('change', clamp); clamp();
    });
  });
})();
